...
StringIntMap makeWordCounts(const StringVec &words) {
    ...
    // =================================================
    for (auto i:words) {
        if (wordcount_map.count(i)) { // if word exists in the wordcount_map
            wordcount_map[i]++;
        } else { // if not exist
            wordcount_map.insert({i, 1});
        }
    }
...
int memoizedLongestPalindromeLength(LengthMemo &memo, const std::string &str, int leftLimit, int rightLimit,
    ...
    if (memo.count(pairKey)) { // check if pairKey exists in memo
    ...
    if (str.at(leftLimit) == str.at(rightLimit)) {
        ...
        int middleSubproblemResult = memoizedLongestPalindromeLength(memo, str, newLeft, newRight, startTime,
                                                                     maxDuration);
    ...
    int leftSubproblemResult = memoizedLongestPalindromeLength(memo, str, leftLimit, rightLimit - 1, startTime,
                                                               maxDuration);
    // Move the left limit to the right and recurse.
    int rightSubproblemResult = memoizedLongestPalindromeLength(memo, str, leftLimit + 1, rightLimit, startTime,
                                                                maxDuration);
